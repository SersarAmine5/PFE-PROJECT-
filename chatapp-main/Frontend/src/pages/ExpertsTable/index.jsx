...
const ExpertTable = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            const response = await axios.get('http://localhost:8800/api/users/usersSubmited');
    ...
    const handleBlockUsers = async () => {
        if (showCheckboxes) {
            const usersToExpert = userData.filter((_, index) => selectedUsers[index]);
            for (let userToExpert of usersToExpert) {
                console.log("[will-modify]:", userToExpert._id); // Mise à jour locale du rôle
                if (userToExpert.role === 'user') {
                    await axios.put(
                        `http://localhost:8800/api/users/users/approuver_expert/${userToExpert._id}`, {
                        role: 'utilisateur expert'
                    }, {
                        withCredentials: true
                    });
                    userToExpert.role = 'utilisateur expert'; // Mise à jour locale du rôle

                    // Supprimer l'utilisateur de la table
                    const index = userData.findIndex(user => user._id === userToExpert._id);
                    if (index > -1) {
